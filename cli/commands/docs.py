...
"""
        ...
        if "error" in response:
            ...
            if not no_stream:
                ...
                print_success(f"Documentation search completed with {response.get('model_used', model)}.")
...
def summarize(
    ...
    try:
        ...
        if use_local:
            local_models = get_available_local_models()
            if not local_models:
                print_warning("No local models available. Falling back to simple summarization.")
                use_local = False
            elif model not in local_models:
                print_warning(f"Model '{model}' not found. Available models: {', '.join(local_models)}")
                if "deepseek-r1:7b" in local_models:
        ...
        if use_local:
            ...
            if length == "short":
                ...
                length_instruction = "Create a medium-length summary in about 1-2 paragraphs."
...
"""
            ...
            if "error" in response:
                ...
                if not no_stream:
                    print("\n")  # Add extra newline for separation
                    print_success(f"Summarization completed with {response.get('model_used', model)}.")
